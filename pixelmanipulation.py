...
# Global variables
global count, emig, panelA, panelB, x, filename, image_encrypted, key
panelA = None
panelB = None
x = None
filename = None
...
# Utility functions to get parts of the file path
def getpath(path):
    a = path.split('/')
    fname = a[-1]
    l = len(fname)
...
def openfilename():
    try:
        ...
        logging.debug(f'File selected: {filename}')
        return filename
...
def en_fun(x):
    ...
    try:
        image_input = cv2.imread(x, cv2.IMREAD_GRAYSCALE)
...
# Function to download encrypted image
def download_encrypted():
    ...
    try:
        ...
        logging.error(f'Failed to download encrypted image: {e}')
        messagebox.showerror("Error", f"Failed to download encrypted image: {e}")

# Function to download decrypted image
def download_decrypted():
    ...
    try:
        ...
        logging.error(f'Failed to download decrypted image: {e}')
        messagebox.showerror("Error", f"Failed to download decrypted image: {e}")
